...
let assert_thunk name to_string thunk =
  printf "%s... " name;
  try
...
let assert_thunk_unequal name to_string thunk =
  printf "%s... " name;
  try
...
let assert_fail name expected got =
  printf "%s... " name;
...
let assert_unequal_int name reference got =
  printf "%s... " name;
  if reference <> got then
    Pass
  else
...
let assert_named_int name got expected_str ok =
  printf "%s... " name;
  if ok then
    Pass
  else
...
let assert_equal_strings name got expected =
  printf "%s... " name;
  if expected = got then
    Pass
  else
...
let expect_opposite i board1 board2 =
  let name = sprintf "compare test %d" i in
    try
      ...
      let test1 =
        assert_named_int (name ^ ", part 1") c1 "1 or -1" (abs c1 = 1)
      in
      let test2 =
        assert_named_int (name ^ ", part 2") c2 "1 or -1" (abs c2 = 1)
      in
      let test3 =
        assert_unequal_int (name ^ ", part 3") c1 c2
      in
      ...
      let results = 
        List.map
          (fun (m, nb) -> 
             ...
             let _ = incr count in
               assert_bool current_name (test_move m b' nb))
          bms
      in
      ...
      let results =
        List.map
          (fun m ->
             let current_name = sprintf "%s, part %d" name (!count + 1) in
             let _ = incr count in
               assert_bool current_name (make_move m b' = None))
          ms
      in
        update_stats (combine_tests results)
    with e ->
